...
export function createTukmokConfig(position: Point, angle: number): EntityConfig {
   ...
   const bodyHitbox = createHitbox(transformComponent, null, new RectangularBox(position, new Point(0, 0), angle, 120, 200), 8, HitboxCollisionType.soft, CollisionBit.default, DEFAULT_COLLISION_MASK, [HitboxFlag.TUKMOK_BODY]);
   addHitboxToTransformComponent(transformComponent, bodyHitbox);

   const headOffset = new Point(0, 118 + 18);
   ...
   const tailOffset = new Point(0, -124);
   const tailPosition = position.copy();
   tailPosition.add(rotatePoint(tailOffset, angle));
   
   const tailConfig = createTukmokTailConfig(tailPosition, angle);
   const childConfigs = [tailConfig];
