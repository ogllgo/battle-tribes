...
export function createRockSpikeConfig(position: Point, rotation: number, size: number, frozenYeti: Entity): EntityConfig {
   ...
   const hitbox = createHitbox(transformComponent, null, new CircularBox(position, new Point(0, 0), rotation, ROCK_SPIKE_HITBOX_SIZES[size]), MASSES[size], HitboxCollisionType.soft, CollisionBit.default, DEFAULT_COLLISION_MASK, []);
