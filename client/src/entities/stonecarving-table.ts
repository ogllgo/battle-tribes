...
export function createStonecarvingTableConfig(position: Point, rotation: number, tribe: Tribe): EntityParams {
   const hitboxes = new Array<Hitbox>();
   let hitboxLocalID = 0;

   const box = new RectangularBox(position, new Point(0, 0), rotation, 120, 80);
   const hitbox = createHitbox(hitboxLocalID++, null, box, new Point(0, 0), 1, HitboxCollisionType.hard, HitboxCollisionBit.DEFAULT, DEFAULT_HITBOX_COLLISION_MASK, []);
   ...
   return {
      ...
      serverComponentParams: {
         ...
         [ServerComponentType.craftingStation]: createCraftingStationComponentParams(CraftingStation.stonecarvingTable)
      },
