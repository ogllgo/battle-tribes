...
import { createFenceGateComponentParams } from "../entity-components/server-components/FenceGateComponent";
...
export function createFenceGateConfig(position: Point, rotation: number, tribe: Tribe): EntityParams {
   const hitboxes = new Array<Hitbox>();
   let hitboxLocalID = 0;

   const box = new RectangularBox(position, new Point(0, 0), rotation, 56, 16);
   const hitbox = createHitbox(hitboxLocalID++, null, box, new Point(0, 0), 1, HitboxCollisionType.hard, HitboxCollisionBit.DEFAULT, DEFAULT_HITBOX_COLLISION_MASK, [HitboxFlag.NON_GRASS_BLOCKING]);
   ...
   return {
      ...
      serverComponentParams: {
         ...
         [ServerComponentType.fenceGate]: createFenceGateComponentParams()
      },
